...
"""Business reconciliation helpers (Track D, Chapter 6).

Chapter 6 treats reconciliations as *data validation*.

This module provides:
- bank reconciliation helpers (bank feed vs book cash activity)
- AR rollforward tie-outs (AR subledger vs trial balance control)

The intent is to keep Chapter 6 code readable and testable.
"""
...
def build_cash_txns_from_gl(gl: pd.DataFrame) -> pd.DataFrame:
    ...
    cash_lines = gl.loc[gl["account_id"].astype(str) == "1000", ["txn_id", "date", "description", "debit", "credit"]].copy()
    if cash_lines.empty:
