...
def test_pyproject_version_is_0240():
    data = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    assert data["project"]["version"] == "0.24.0"
...
def test_release_notes_have_a_root_title_then_book1_entry():
    ...
    assert "pystatsv1 book1 init" in text
    assert "synthetic-only executable companion" in text
...
def test_book1_launcher_docs_pin_the_matching_release_and_boundary():
    ...
    assert 'pystatsv1[book1]==0.24.0' in text
